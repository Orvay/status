...
var probeTarget = async (target, fetchImpl, clock, delay = sleep, probeSecret) => {
...
var renderIncidentPage = (input) => {
  ...
  const SLICES = 90;
  const phaseAt = (t) => {
    let current = incident.phases[0];
    for (const phase of incident.phases) {
      if (phase.at <= t) current = phase;
    }
    return current;
  };
  const segments = Array.from({ length: SLICES }, (_, i) => {
    const t = start + span * i / (SLICES - 1);
    const phase = phaseAt(t);
    const anchor = i < 8 ? ' data-tip="start"' : i > SLICES - 9 ? ' data-tip="end"' : "";
    return `<span class="seg" tabindex="0"${anchor} style="background:${IMPACT_TINT[phase.impact]}"><span class="tip"><span class="tip-day">${esc(utc(t))}</span><span class="tip-state" data-s="${phase.impact}">${esc(IMPACT_LABEL[phase.impact])}</span></span></span>`;
  }).join("");
...
`),
    ...
    writeFile(
      ...
      renderHistoryPage({ tokensCss: tokens_default, pageCss: page_default, incidents, generatedAt: at })
    ),
    writeFile(
