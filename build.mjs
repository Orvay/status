...
var historyBar = (series, endDay, label) => {
  ...
  const cells = series.map((state, i) => {
    ...
    return `<span class="cell${state === void 0 ? " cell--none" : ""}" tabindex="0"${anchor}${state === void 0 ? "" : ` style="background-color:${DAY_TINT[state]}"`}><span class="tip"><span class="tip-day">${esc(prettyDay(key))}</span><span class="tip-state" data-s="${dot}">${esc(word)}</span></span></span>`;
  }).join("");
...
// src/build.ts
var sourceCommit = true ? "42fa410" : "unknown";
